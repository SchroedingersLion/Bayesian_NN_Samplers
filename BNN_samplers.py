...
HMC - Hamiltonian Monte Carlo, see eg. Neal 2012. It can also be used with stochastic gradients (not to be
      confused with SGHMC).
AMAGOLD - Metropolized Langevin dynamics using stochastic gradients, see Zhang et al., 2020.


train_optimizer - Function to train the model conventionally by using the passed optimizer (such as torch.optimizer.SGD).


Open issues: 1) The kinetic energies and L2 norms are computed layer-wise. Each column corresponds to one layer.
             ...
             2) Loss criterions are assumed to be passed with reduction mode "mean".
"""
...
class HMC(nn.Module):
    ...
    def MH_decision(self, U0, K0, squeeze):
        ...
        MH = np.exp( -1 * (U1 - U0 + K1-K0) )    # evaluate MH-term
...
class AMAGOLD(nn.Module):
    ...
    def __init__(self, model, train_loader, test_loader, criterion, lr, gamma, weight_decay, epochs, L, resample):
        ...
        self.BBK_fac1 = 1 - self.lr * self.gamma/2      # constants used in integrator
        self.BBK_fac2 = 1 / (1 + self.lr * self.gamma/2)
        self.noise_fac = np.sqrt(2*self.lr*self.gamma)
    ...
    def train(self):
        ...
        for p in self.model.parameters():                   # create momentum buffers
            p.buf = torch.randn(p.size()).to(device) 
            mom_old += [p.buf.clone()]                      # needed to store old momenta for energy accumulator rho
        ...
        start_time = time.time()
        print("Starting AMAGOLD sampling... \nPerform {} epochs at batch size {} and L={},"\
              " i.e. {} outer loop iterations.".format(self.epochs, self.train_loader.batch_size, self.L, N))     
        ...
        for n in range(1,N+1):  
            ...
            if self.resample == True:
                for p in list(self.model.parameters()):                 # resample momentum
                    p.buf = torch.randn(p.size()).to(device)            
            ...
            for batch_idx, (data, target) in enumerate(self.train_loader):
                ...
                for p in list(self.model.parameters()):
                    p.data.add_(p.buf, alpha=self.lr)                               # parameter update
                    
                data, target = data.to(device), target.to(device)                   # compute new gradients
                self.fill_gradients(data, target, squeeze, datasize)
                
                self.update_momenta(mom_old, rho)                                   # update momenta (and rho)
            
           
            ## METROPOLIS           
            (accept, U1) = self.MH_decision(U0, rho, squeeze)
            # accept = True; U1=1
            # print("always accept!!")
            ...
            if n%5==0:
                print("AMAGOLD ITERATION {} DONE!".format(n))
    ...
    def MH_decision(self, U0, rho, squeeze):
        """
        Evaluates Metropolis criterion. Returns boolean (true=accept, false=reject) and new potential energy.
        """
        ...
        MH = np.exp( -1 * (U1 - U0 - rho) )    # evaluate MH-term
    ...
    def update_momenta(self, mom_old, rho):
        """
        Momentum update for all parameters. Also updates \rho, the energy accumulator used for MH criterion.
        """
        
        for (idx,p) in enumerate(list(self.model.parameters())):               
            
            mom_old[idx].copy_(p.buf)                                       # store old momentum
            
            eps = torch.randn(p.size()).to(device)                          # momentum update
            p.buf.mul_(self.BBK_fac1)
            p.buf.add_(eps, alpha=self.noise_fac) 
            p.buf.add_(p.grad.data, alpha=-self.lr)
            p.buf.mul_(self.BBK_fac2)                     
            
            mom_old[idx].add_(p.buf)                                        # add both momenta
            mom_old[idx].mul_(p.grad.data)                                  # scalar product with force
            
            rho += 0.5 * self.lr * mom_old[idx].sum()                       # update rho
